...
afterglow then the host galaxy can be identified, from which a redshift can
...
The range to which the \emph{Swift} x-ray telescope has detected \acp{sGRB} is
...
observations of \ac{sGRB} events. Various studies using the first two
...
rate is that given in Table I of \cite{2013arXiv1304.0670L} for an \ac{aLIGO}
...
\emph{post facto} matching of \ac{GW} trigger lists with known (or subsequently found
...
modeling our signal (in both simulations and signal recovery).  For \ac{BNS}
systems we use nonspinning waveforms under the assumption that the component spins will be small 
and have a negligible effect on the analysis\footnote{In future studies we could create our 
simulations including small spins and confirm that this does not have a significant effect on our 
results if recovery is with nonspinning templates.} (see discussion in \cite{2014ApJ...795..105S} 
...
For a nonspinning system the general form of the frequency-domain polarization
amplitudes is
%
...
are dependent upon the polarization angle $\psi$ and the sky position of the
...
We consider that the measured data in any detector are defined as
%
\begin{equation}
...
frequencies used for our analysis, which were 20 to 400\,Hz respectively.
...
or higher frequencies provides very little additional information while increasing
...
polarized), which allows us to place prior constraints on the system
...
of polarization angle prior range given below, explicitly fixes the handedness
...
for back on), or alternatively to double the range of the polarization angle,
...
the normalized aligned spin magnitude between $-1$ and $1$.
...
value and, along with the polarization angle, defines the antenna response patterns.
...
polarization angle we use a uniform prior on the range $(0,\pi/2]$. As the
signals we use are generally close to being circularly polarized (face on) the
phase and polarization angle will be largely degenerate and their exact
...
data will be close to being correct, so we want to use a prior that is peaked at $\scf_k=1$. We
...
favors \scf\xspace values greater than 1), but given our simulation criterion
described in Sec.~\ref{sec:simulations} we find that the likelihood
...
at a range of distances: 50 to 500\,Mpc with 50\,Mpc increments for the
\ac{BNS} signals and 100 to 900\,Mpc with 100\,Mpc increments for the
...
domain and spanned a frequency range from 20 to 400\,Hz, which is a range that contains the 
vast majority of the \ac{SNR} and is therefore all that is required to provide good 
constraints on \scf. We assumed all
detectors were operating at their design sensitivity (as given by Fig.~1 of
\cite{2013arXiv1304.0670L}). As such we added colored Gaussian noise to each
...
Sec.~\ref{sec:priors}, with the sky position being drawn randomly from a
uniform distribution on the sky, the coalescence times held fixed within the
center of the prior window, and with the calibration scaling factors for each
...
uncertainties that is expected via experimental methods, but is not the same as
...
In accepting a proposed simulation as one to be analyzed we introduced a
criterion that the signal be ``detectable'', based on that used in
\cite{2012PhRvD..85h2002A}. We therefore only analyzed simulations that had an
...
sources was not uniformly distributed over the sky and instead favored locations with better
...
very close to circularly polarized.
...
SNR}$ for each detector. As we show below this assumption is reasonable,
...
Sec.~\ref{sec:priors}. In each case when calculating the likelihood we used
...
64 separate noisy realizations of the \ac{PSD} and scaled
...
the marginalized posterior probabilities for a \ac{BNS} system and a \ac{NSBH}
system observed with the three detector network are shown in
Figs.~\ref{fig:bnspost} and \ref{fig:nsbhpost} respectively.
...
true value. These are shown as boxplots in Figs.~\ref{fig:bnsresults} and
\ref{fig:nsbhresults} for the \ac{BNS} and \ac{NSBH} systems respectively. The
boxes show the extent from the lower to upper quartile of the values, while
the whiskers extend from the fifth to 95$^{\rm th}$ percentile. The
...
signals drawn from the prior distribution that fulfils the detection criterion.

\begin{figure*}
 \begin{center}
  \includegraphics[width=1.0\textwidth]{bns_post_fig.pdf}
 \end{center}
 \caption{\label{fig:bnspost} The marginalized posterior probability distributions for some of the unknown
...
\begin{figure*}
 ...
 \end{center}
 \caption{\label{fig:nsbhpost} The marginalized posterior probability distributions for some of the unknown
...
scale factors can be determined for a three detector network using
\ac{BNS} systems (provided a coincident GRB is observed and can yield a
...
(provided a coincident GRB is observed and can yield a distance estimate). The plot contents 
are the same as in Fig.~\ref{fig:bnsresults}.}
\end{figure*}

% discuss the BNS plots - general behaviour
In Fig.~\ref{fig:bnsresults} we see that on average the scale factor can be
...
maximum at $\sim 25\%$, while the width of boxes narrows. This is due to our
``detectibility'' criteria, where for all distances we only see
...
between the scale factors for H1 and L1 as seen in Fig.~\ref{fig:bnspost}.

% discuss the NSBH sources
In Fig.~\ref{fig:nsbhresults} we see very similar results for the \ac{NSBH}
...
the population of sources that is detectable. For distances at which the
...
in Fig.~\ref{fig:nsbhresults}, but again extending to further distances. However, at higher 
...
% discuss any bias on the actual recovered values
\subsection{Parameter biases}

It is also interesting to see if our analysis has any bias on the recovered \scft \acp{PDF}, or whether they
...
for the \ac{BNS} systems at 50 and 500\,Mpc (see Fig.~\ref{fig:ppplots}).
...
Figure~\ref{subfig:pp50} shows
that for sources at 50\,Mpc there is no highly significant bias on the recovery of \scft for any of
the detectors. However, in Fig.~\ref{subfig:pp500}, with sources at 500\,Mpc, while
H1 and L1 follow the diagonal well, V1 goes significantly above the diagonal.
This shows that the \acp{PDF} on \scft for V1 are too broad. The reason for this is that at these
larger distances the \acp{PDF} on \scft for V1 are not well constrained and start to become dominated
by the prior. As we have previously noted this prior is not the same as the distribution used
to generate the \scft values, but is instead far broader and more conservative. So this bias is
showing up due to the simulated values of \scft being generated from a much more narrow range.

Despite the inconsistency in V1 it is satisfying to see that for the detectors for which the
\scft values are reasonably constrained (i.e.\ H1 and L1 in Fig.~\ref{fig:bnsresults}) the
actual value is recovered consistently, with little effect from the prior.

\begin{figure*}
\begin{center}

\begin{tabular}{c c}
\subcaptionbox{\label{subfig:pp50}}{
 \includegraphics[width=0.475\textwidth]{pp50Mpc}}

&

 \subcaptionbox{\label{subfig:pp500}}{
 \includegraphics[width=0.475\textwidth]{pp500Mpc}}
\end{tabular}
\end{center}
\caption{\label{fig:ppplots} Both figures show the cumulative fraction of true \scft values
found within a
given fractional credible interval versus the fractional credible interval for each detector for
the \ac{BNS} simulations at (\subref{subfig:pp50}) 50\,Mpc and (\subref{subfig:pp500})
500\,Mpc. The grey shaded region is a 95\% credible band for the expected deviations from diagonal.}
\end{figure*}
...
reasonably accurate in addition to the existing calibration methods \cite{2010NIMPA.624..223A}.
We acknowledge that we have made a number of simplifying assumptions, 
...
calibration.  We have also neglected any phase uncertainty in the calibration.



% comparative calibration
A further application of astrophysical calibration is the relative calibration
of \ac{GW} detectors.  In this case we are interested in obtaining
...
We emphasize that our work has assumed that the \ac{PSD} used in the likelihood
...
then the \ac{SNR} contribution in each frequency section is of relevance.
...
being used to marginalize over calibration uncertainties, but do not yet
...
calibration assessment may not be possible. However, there is at least one currently known 
...
We acknowledge useful discussions within the LIGO-Virgo
Collaboration and specifically with Salvatore Vitale. L.W.\ was part funded for
this work through a Royal Astronomical Society Undergraduate Research Bursary.
M.P.\ and C.M.\ are funded by the Science and Technology Facilities Council (STFC) under Grant.
No.\ ST/L000946/1. C.M.\ is also supported by a Glasgow University Lord Kelvin
Adam Smith fellowship. We are grateful for computational resources provided by
Cardiff University, and funded by STFC Grant No.\ ST/I006285/1 supporting United Kingdom involvement in
the operation of Advanced LIGO. This document has LIGO DCC Report No. \dcc.
