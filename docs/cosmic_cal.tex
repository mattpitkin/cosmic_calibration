...
counterpart) are derived from three main sources, population sythesis models,
...
year~\cite{2013arXiv1304.0670L}~\cite{2010CQGra..27q3001A}~\footnote{We should
briefly mention why the rates differ by a factor of 2.  I can't remember right
now.}. In~\cite{2012MNRAS.425.2668C} using the rate of observed \acp{sGRB} and
...
year~\cite{2013PhRvL.111r1101C},~\cite{Kelley:2012fl},~\cite{2014arXiv1405.2254S},~\cite{2015MNRAS.448.3026W}.  
...
from the \ac{sGRB} to within a few seconds~\cite{grb}.  This makes the follow-up search less 
...
performed simulations of multiple events to see the expected distribution in
the accuracy of the calibration scale factor recovery.

% Describe the signal model (Taylor F2 etc...)
We use the TaylorF2 waveform approximation (see e.g.~\cite{2009PhRvD..80h4043B}
...
systems we use non-spinning waveforms under the assumption that spins will be
negligible~\cm{ref?}. For \ac{NSBH} system we use a spinning waveform, but in which only
the black hole has non-negligible spin and that its rotational angular momentum
is aligned with the system's orbital angular momentum.
...
\left(\cos^{2}\iota\right)D_{L}^{-1}\mathcal{M}^{5/6} f^{-7/6}e^{-i\Psi(f, \nu,
...
the inclination angle, and $\nu=(\pi Mf)^{1/3}$.  The \ac{GW} strain measured
...
For this analysis we will consider the advanced generation \ac{aLIGO} and
...
$\scf_k$ is the calibration scale factor for the $k^{\rm th}$ detector, the
...
provide a very tight constraint on the source distance\footnote{For \ac{BNS}
and \ac{NSBH} systems they will be observable only in the relatively nearby
Universe ($z \sim 0.1$ and $z\sim 0.2$ for \ac{BNS} and \ac{NSBH} systems
respectively), so the relation between the measured \ac{sGRB} redshift and
distance is simple and our luminosity distance will be negligibly effected by
the choice of cosmology \MP{Check this?}.}. In this analysis we therefore
...
prior on the inclination angle with zero mean and a standard deviation
$\sigma=14.8^{\circ}$ truncated at values below zero.
...
$0.13\,\textrm{M}_{\odot}$ \MP{(reference)}. In reality, for all the systems we
use in our study, the chirp mass, which we see from Eq.~\ref{eq:signal}
...
equivelent to $p(\scf|I)=0.1$. For the scale factor in each detector we 
...
prior does not give a symmetric amount of probability about unity, but given
...
domain and spanned a frequency range from 20\,Hz to 400\,Hz~\cm{can you remind
me and maybe the reader of why a 400 Hz cutoff is OK?}. We assumed all
...
number~\cm{how many exactly/roughly?} simulations at each distance value. When
...
antenna response.  Similarly, despite already constrained by the prior to small
...
sensitivities (using those given in~\cite{2013arXiv1304.0670L}), but simulated
as if calculated by averaging 64 separate noisy \ac{PSD} estimates and scaled
with the same factor as applied to the injection and noise~\cm{Do you estimate
the PSD using 64 segments? This sentence might be a bit confusing to the
reader.}\footnote{We do not account for there being a potential difference
...
of the detectors respectively.~\cm{The fonts are a bit small.  Maybe also
highlight somehow the boxes refereing to the scale factors?}}
\end{figure*}
...
the detectors respectively.~\cm{same things about the fonts and highlighting
the scale factors.}}
\end{figure*}
...
The largest \ac{SNR} contribution will generally come from either of the
\ac{aLIGO} detectors and thus the detection criteria (\ac{SNR} threshold) will
not apply to to\ac{adV} result. Hence, the \ac{SNR} in \ac{AdV} can be small
...
in figure~\ref{fig:nsbhresults}, but again extending to further distances.
...
with frequency.  Other authors~\cm{Ref: Cornish? and others?} have addressed
...
still offer an upper limit on the calibration uncertainty. However, future
