...
\sim 0.67\sigma$. It follows that a median opening angle of a $\sim 10^{\circ}$
corresponds to $\sigma \sim 14.8^{\circ}$. We therefore place a Gaussian
...
to produce results domintaed by the likelihood for these parameters.}. 
...
with the same calibraion scale factor as applied to the injection and noise\footnote{We do not account for there 
...
by the prior. As we have previously noted this prior is not that same as the distribution used
