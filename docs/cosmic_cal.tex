...
spins will be negligible. For \ac{NS-BH} system we use a spinning waveform, but in which only the 
...
Mf)^{1/3}$.  The \ac{GW} strain measured at the $k^{\textrm{th}}$ detector is then given by
%
\begin{equation}
  \label{eq:gravsig}
...
our analysis, which were 30\,Hz to 1600\,Hz respectively. The source parameters, and the 
unknown scale factor, have been subsumed into a vector $\btheta$. For a network of detectors, 
