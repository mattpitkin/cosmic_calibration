...
\refresp{\cite{2010CQGra..27q3001A}}. One such class of these cataclysmic
events, the inspiral and merger of \ac{BNS} systems will be detected out to a
maximum range \refresp{(horizon distance)} of $\approx 450$ Mpc. Assuming the
...
\acp{sGRB} with \ac{GW} events are the estimated astrophysical rates of both
phenomena. Observations of \acp{sGRB} give an inferred rate of \ac{sGRB}
producing \ac{CBC} mergers of $8\times 10^{-9}$--$1.1\times
10^{-6}$\,Mpc$^{-3}$\,yr$^{-1}$~\cite{2012MNRAS.425.2668C} \refresp{after accounting for
...
1\%$ accuracy~\refresp{\cite{2015arXiv150201589P}}.  The \ac{GW} measurement
...
we can re-calibrate (or validate) the existing experimentally obtained
calibration. 
...
Fabry-P\'{e}rot Michelson interferometer\refresp{,} where an elaborate feedback
...
frequency range used in this analysis \refresp{(20--400 Hz)}, the error in the
\refresp{strain} amplitude calibration is expected to be roughly $10\%$
...
\refresp{each of which are subject to measurement uncertainties}. In this study
we will assume an estimate of $R$ is available (although in theory we could
...
\refresp{(i.e.\ the measured length response function $R(f)$ is related to the
...
would appear to have a larger amplitude \refresp{(i.e.\ for a given system,
...
reality.  In this analysis we will simplify the situation by assuming that
...
could take some functional form, or piece-wise fit, with respect to $f$ (e.g.\
...
the \refresp{global network of \ac{GW} detectors}. However, measurement of the
...
the 3rd generation \ac{GW} interferometer, the
\ac{ET}~\refresp{\cite{2010CQGra..27h4007P} and in ~\cite{2012PhRvD..85b3535T}
...
\refresp{form} of \ac{NS} mass and spatial distributions or exploit the
features of the tidal and post-merger hyper-massive \ac{NS} stages of the
\ac{GW} waveform. 
...
is has uncertainties of $O(1\%)$~\refresp{\cite{2015arXiv150201589P}}) is
...
${\sim}1/3$~\refresp{\footnote{http://swift.gsfc.nasa.gov/archive/grb\_table/}}.
...
counterpart) are derived from three main sources\refresp{:} population
...
the expected rate by a factor of five.}. \refresp{A similar event rate of
...
\refresp{For the scenario of a \ac{GW}-followup of an observed \ac{sGRB}, the
merger time for \ac{BNS}/\ac{NSBH} systems will be estimated from the \ac{sGRB}
to within a few seconds (see e.g.\ 340 the discussion in Sec.\ 2.2 of
...
limits us to systems that are approximately ``face-on'' \refresp{and therefore
effectively optimally oriented. At given distances this biases us to  higher
...
250\,Mpc, had \ac{SNR} of 7.9, 9.2 and 5.2 and percentage relative uncertainties in the calibration scale 
factors of 17\%, 15\% and 24\% for each of the detectors respectively.}
\end{figure*}
...
(H1, L1 and V1). The simulated signal was at a distance of 450\,Mpc, had \ac{SNR} of 9.8, 10.3 and 
3.8 and percentage relative uncertainties in the calibration scale factors of 15\%, 14\% and 36\% for each of 
...
the detectors. However, in figure~\ref{subfig:pp500}, with sources at 500\,Mpc, whilst
...
by the prior. As we have previously noted this prior is not \refresp{the} same as the distribution used
to generate the \scft values, but is instead far broader and more conservative. So, this bias is
showing up due to the simulated values of \scft being generated from a much narrow range.

Despite the inconsistency in V1 it is satisfying to see that for the detectors for which the
\scft values are reasonably constrained (i.e.\ H1 and L1 in figure~\ref{fig:bnsresults}) the
...
\end{tabular}
\end{center}
\caption{\label{fig:ppplots} Both figures shows the cumulative fraction of true \scft values
found within a
given fractional credible interval versus fractional credible interval for each detector for
