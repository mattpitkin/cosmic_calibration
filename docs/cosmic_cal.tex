...
momentum vector of the \ac{BNS} system. Additional evidence for the coincidence
...
\cite{2010CQGra..27q3001A}. However, the recent work of
\cite{2015MNRAS.448..928K}, which studied known galactic \ac{BNS} systems in
more detail suggest a rate approximately five times lower. The estimates for
...
possible to identify the host galaxy of the \ac{BNS} source.  With this
...
orbital parameters allow them to be guaranteed sources for such detectors. These too could be used 
for assessing calibration for such detectors. However, generally for these verification binaries 
...
\cite{2041-8205-737-1-L23}, which make it an excellent candidate for an astrophysical calibrator at 
its frequency.
...
the rate of observed \acp{sGRB} and assuming a beaming angle of $15^{o}$ a
...
uncertainties that are expected via experimental methods but not the same as
the prior that we assume on $\mathcal{C}$ (see Eq.~\ref{eq:scale_prior}) when
...
antenna response.  Similarly, despite being already constrained by the prior to
