...
\begin{abstract}
  We present an investigation assessing the validity of the 
of gravitational wave detector calibration through the use of gamma-ray
bursts as standard
  ...
  a direct comparison with the estimated value and discrepancies can
  then be used to correct the original calibration.~\MP{Update with results.
End with a statement comparing to existing method but stressing its independent.}
\end{abstract}
...
candles). \cm{Standard sirens allow ... fix me}
...
give a rate of \cm{XXX}\,Mpc$^{-3}$\,yr$^{-1}$ which can be compared to estimates of \ac{BNS} merger 
...
\MP{Move this paragraph to conclusions} For future planned space-based detectors such as eLISA there are several known galactic binary 
...
\cm{Lose, trim or merge this paragraph with previous one.} The use of \ac{GW} events with \ac{sGRB} counterparts for use as a cosmological tool was 
...
year~\cite{2013arXiv1304.0670L,2010CQGra..27q3001A}\footnote{\cm{Email Patrick
S about this} We note that this rate is that given 
...
$\sigma=14.8^{\circ}$.~\MP{Check regarding the face/arse orientation issue.}
...
favour \scf\xspace values greater than 1), but given our simulation criterion
...
however, this method would provide a \MP{semi-independent} consistency check that
...
\cm{Chris: feel free to add a short paragraph on relative calibration here.}
