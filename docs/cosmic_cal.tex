...
It is considered likely that the merger of \ac{BNS} and/or \ac{NSBH} systems, in addition to 
emitting detectable \acp{GW}, is also the mechanism for producing \acp{sGRB} 
\cite{1992ApJ...395L..83N}. In this scenario these events produce tightly beamed \ac{EM} emission 
parallel to the orbital angular 
...
give a rate of XXX\,Mpc$^{-3}$\,yr$^{-1}$ which can be compared to estimates of \ac{BNS} merger 
rates of $10^{-8}$--$10^{-5}$\,Mpc$^{-3}$\,yr$^{-1}$ \cite{2010CQGra..27q3001A}, or \ac{NSBH} merger 
rates of $6\,\times\,10^{-10}$--$1\,\times\,10^{-6}$\,Mpc$^{-3}$\,yr$^{-1}$ 
\cite{2010CQGra..27q3001A}. However, the recent work of \cite{2015MNRAS.448..928K}, which studied 
known galactic \ac{BNS} systems in more detail suggest a rate approximately five times lower. The 
estimates for \ac{NSBH} systems come entirely from population synthesis models as no such systems 
have been observed.
%The ranges of rates for \acp{BNS} is obtained from population synthesis models together with 
%knowledge of the X known \ac{BNS} systems in our galaxy, whilst the range for \acp{NSBH} is 
%entirely from population synthesis as not such systems have been observed. 
...
can be very accurately obtained (see values quoted in e.g. \cite{0004-637X-664-2-1000} and 
references therein). With knowledge of the redshift, using current best 
estimates for the Hubble constant and other cosmological parameters, the true luminosity distance 
can be estimated to $\sim 1\%$ accuracy.  The \ac{GW} measurement acts as a standard siren also 
giving us a direct measurement of the luminosity distance to the source. The accuracy of such a 
measurement depends on a number of factors including the accuracy with which the \ac{GW} detector 
has been calibrated.  Hence, by comparison with the distance estimate from the \ac{sGRB} we can 
re-calibrate (or validate) the existing experimentally obtained calibration. 

For future planned space-based detectors such as eLISA there are several known galactic binary 
systems known as ``verification'' binaries \cite{2006CQGra..23S.809S}, in that their masses and 
orbital parameters allow them to be guarenteed sources for such detectors. These too could be used 
for assessing calibiration for such detectors. However, generally for these verification binaries 
the inclination of the system is not known and distances are fairly uncertain, so an accurate 
calibration assessment may not be possible. Although there is at least one (J0651) currently known 
eclipsing white dwarf binary \cite{2041-8205-737-1-L23}, which does provide the system inclination 
\cite{2012ApJ...757L..21H, 2013ASPC..467...47K} and has an approximately 10\% distance uncertainty 
\cite{2041-8205-737-1-L23}, which make it an excellent candiate for an astrophysical calibrator at 
its frequency.
...
discuss out analysis method and the results, and in Sec.~\ref{sec:discussion} we 
conclude.    
...
amplitude calibration is roughly $10\%$ \cite{Vitale:2012}. This is a benchmark set for the error 
estimation using the proposed method in this project.
...
The use of \ac{GW} events with \ac{sGRB} counterparts for use as a cosmological tool was 
investigated in~\cite{2010CQGra..27u5006S} and~\cite{2011PhRvD..83b3005Z} with respect to
...
inference (see~\cite{1996PhRvD..53.2878F,2012PhRvD..85b3535T, 2012PhRvD..86b3502T}) have 
been proposed for \ac{BNS} systems that do
...
equation-of-state is sufficiently well known. This development, and separately, the method
...
year~\cite{2013arXiv1304.0670L,2010CQGra..27q3001A}\footnote{We note that this rate is that given 
in Table 1 of \cite{2013arXiv1304.0670L} for an \ac{aLIGO} and \ac{AdV} network consisting to two 
\ac{aLIGO} detectors operating at design sensitivity. However, a higher rate of $0.4$--$400$ is 
given in \cite{2010CQGra..27q3001A}, which may differ due to the use of a third \ac{aLIGO} 
detector, but the discrepancy may also have contributions from slightly different threshold 
definitions or design sensitivity curves used. We also note the study mentioned in 
Sec.~\ref{sec:intro} that potentially lowers the expected rate by a factor of five.}. 
In~\cite{2012MNRAS.425.2668C} using the rate of observed \acp{sGRB} and
assuming a beaming angle of $15^{o}$ a similar event rate of $1$--$180$ is
obtained. The beaming angle is the primary factor in determining the fraction
of detected \ac{GW} \ac{BNS} signals with an \ac{EM} \ac{sGRB} counterpart and
is unfortunately highly uncertain.  Various authors have provided estimates on
the rate of joint detections to be in the range $0.02$--$7$ per
...
defined in Eqn.~\ref{eq:scalefactor} can be estimated from a single observed
...
with the symmetric mass ratio $\eta=m_{1}m_{2}/M^{2}$.  The total mass
$M=m_{1}+m_{2}$ where $m_{1}$ and $m_{2}$ are the component masses, $\iota$ is
the inclination angle.  The \ac{GW} strain measured
...
prior).~\cm{Should we not say a little bit more about this? A really accurate
distance from a really accurate redshift is only as good as the Hubble constant
which currently has $\sim$ few percent error?  We should also say something
about peculiar motion.}
...
study of estimates and lower-limits of opening angles in 13 \acp{sGRB} given
in~\cite{2014ApJ...780..118F} provide a median opening angle value of $\sim
...
canonical masss distribution used for the rate results in
...
from the identification of the host galaxy. 
...
data will be correct, so want to use a prior that is peaked at $\scf_k=1$. We
...
prior does not give a symmetric amount of probability about unity (so out prior favour \scf\xspace 
values greater than 1), but given
our simulation criterion described in Section~\ref{sec:simulations} we find
that the likelihood generally overwhelms the prior~\cm{I'm not sure about this
sentence.  Why does the symmetry matter?}. As $\scf$ is a scale factor we also
have the further constraint that it must be positive and hence the log-normal
prior is a natural choice.
...
number (several hundred up to $O(1000)$) simulations at each distance value. When
proposing injections for each simulation the source parameters were randomly
drawn from the prior distributions described in Section~\ref{sec:priors}, with
the sky position being drawn randomly from a uniform distribution on the sky,
the coalescence times held fixed within the centre of the prior window, and
with the calibration scaling factors for each detector drawn from a Gaussian
distribution with a mean of one and standard deviation of $0.125$ (equivalent
to a mean fractional offset of 10\%)~\cm{should we explicitly acknowledge that
we do not simulate from the same priors that we assume in the recovery?}.
However, in accepting a proposed injection as one to be analysed we introduced
a criteria that the signal be ``detectable'', based on that used in
...
antenna response.  Similarly, despite being already constrained by the prior to small
inclination angles, the accepted injections show preference for sources very
close to circularly polarised.
...
able to constrain the calibration scale for a given detected \ac{CBC}-\ac{sGRB}
coincidence.
...
an upper limit on the calibration scale factor.~\cm{Even though we're hiding
this in a footnote is it really this bad. How big is the statistical error on
PSD estimation using 64 segments?}}. This has provided posterior probability
...
signals drawn from the prior distribution that fulfil the detection criterion.
...
``detectability'' criteria, where for all distances we will only be seeing
...
\acp{SNR} of the \ac{NSBH} systems over the \ac{BNS} systems~\cm{can we go any
further with this?}.
...
data from the 3 inteferometer advanced detector network. We find that for
...
however, this method would provide a semi-independent consistency check that
the calibration is reasonably accurate.  We acknowledge that we have made a
number of simplifying assumptions, including that the waveforms we use
accurately model the true signal, and that the overall calibration remains
constant over the duration of a signal and its \ac{PSD} estimation. A further
...
\MP{Chris: feel free to add a short paragraph on relative calibration here.}

% discus PSD uncertainty
...
In this case Bayesian model selection may be applicabl for selecting the
...
MP is funded by the STFC under grant number ST/L000946/1 and CM is funded via
XXX. This document has LIGO DCC number \dcc.
