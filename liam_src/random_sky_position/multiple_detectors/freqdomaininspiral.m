...
%Updated version: This version is able to create a frequency domain signal
%for more than one detector. Outputs hf as a matrix of length(fs) by
%length(det).

% global hp hc were not updating when running through with fbins(idx). They
% seems to work as local variables
...
%   scaleint = interp1(scalefreqs, log10([sf1,sf2]), fs, 'spline');
%     scaleint = log10(sf1);
...
%     fc = zeros(1,length(det));
    hc = zeros(length(fs), length(det));
    hp = zeros(length(fs), length(det));
    %fp = zeros(1, length(det));
    %fc = zeros(1,length(det));
...
%         fp = interp1(resplut{j}(1,:), resplut{j}(2,:), psi);
%         fc = interp1(resplut{j}(1,:), resplut{j}(3,:), psi);
        
        hf(idx,j) = amp.*cos(phasing) - 1i.*amp.*sin(phasing);
...
%         [fp(:,j), fc(:,j)] = antenna_pattern(det{j}, ra, dec, psi, tc);         
        hc(:,j) = 1i*hf(:,j).*fc;
        hp(:,j) = hf(:,j).*fp;
    end
end
