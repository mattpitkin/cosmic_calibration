...
mds = zeros(size(md));
logL_multi = zeros(size(Cs));
logL = zeros(length(logL_multi),1);
...
for i = 1:length(C)
    ...
    dh(:,i) = real((conj(ys(:,i)) ./ Cs(:,i)).*mds(:,i)); % model crossed with data
    hh(:,i) = real((conj(mds(:,i)) ./ Cs(:,i)).*mds(:,i)); % model crossed with model
    dd(:,i) = real((conj(ys(:,i)) ./ Cs(:,i)).*ys(:,i)); % data crossed with data
    
    logL_multi(:,i) = 4*df.*(dh(:,i) - 0.5.*(hh(:,i)+dd(:,i))); % finds the log likelihood
    logL(i,:) = sum(logL_multi(i,:));
end
