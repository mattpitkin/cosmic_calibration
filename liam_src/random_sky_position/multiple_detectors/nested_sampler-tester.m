...
% Create 2 cells for to be used for both nest_samples cells
% This is a quick fix!

livepoints = cell(1, 2);
for i = 1:length(livepoints)
    livepoints{i} = zeros(Nlive, D);
end


for j = 1:length(livepoints)
    for i=1:D
        ...
        if strcmp(priortype, 'uniform')
            livepoints{j}(:,i) = p3 + (p4-p3)*rand(Nlive,1);
        elseif strcmp(priortype, 'gaussian')
            livepoints{j}(:,i) = p3 + p4*randn(Nlive,1);
        elseif strcmp(priortype, 'jeffreys')
            % uniform in log space
            livepoints{j}(:,i) = 10.^(log10(p3) + (log10(p4)-log10(p3))*rand(Nlive,1));
        end
    end
end
...
% usings livepoints{1} to find parvals as for the moment, both cells should
% be the same.
for i=1:Nlive
    parvals = cat(1, num2cell(livepoints{1}(i,:)'), extraparvals);
...
for j = 1:length(livepoints)
    for i=1:Nlive
        livepoints{j}(i,:) = scale_parameters(prior, livepoints{j}(i,:));
    end
end
% initial tolerance
tol = [inf inf];
...
nest_samples = cell(1,2);
for i = 1:length(nest_samples)
    nest_samples{i} = zeros(1,D+1);
end
